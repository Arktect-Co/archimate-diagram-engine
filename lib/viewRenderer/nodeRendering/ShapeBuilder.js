const joint = require("jointjs");

class ShapeBuilder {
    constructor({defaultWidth = 140, defaultHeight = 50}) {
        this.defaultWidth = defaultWidth;
        this.defaultHeight = defaultHeight;
    }

    getAttributes_(elementName, fillColor, strokeColor, withDashedStroke, textAnchor, refX) {
        const attributes = {
            body: {
                fill: fillColor,
                stroke: strokeColor,
                rx: 0,
                ry: 0,
                strokeWidth: 0.8,
            },
            label: {
                textWrap: {
                    text: elementName,
                    width: '70%', // reference width minus 10
                    height: '100%', // half of the reference height
                    ellipsis: true
                },
                fill: "black",
                textVerticalAnchor: "top",
                textAnchor: textAnchor,
                refX: refX,
                refY: 10,
                refWidth: '100%',
                refHeight: '100%',
                fontSize: 12,
            },
        };

        if (withDashedStroke) {
            attributes.body.strokeDasharray = '5,5';
        }

        return attributes;
    }

    buildBasicRetangular(elementName, {
        width = this.defaultWidth,
        height = this.defaultHeight,
        textAnchor = "middle",
        refX = "42.5%",
        fillColor = "white",
        strokeColor = "black",
        withDashedStroke = false
    }) {
        let element = new joint.shapes.standard.Rectangle();
        let attributes = this.getAttributes_(
            elementName,
            fillColor,
            strokeColor,
            withDashedStroke,
            textAnchor,
            refX
        );

        attributes.body.rx = 0;
        attributes.body.ry = 0;

        element.resize(width, height);
        element.attr(attributes);

        return element;
    }

    buildBasicRounded(elementName, {
        width = this.defaultWidth,
        height = this.defaultHeight,
        textAnchor = "middle",
        refX = "42.5%",
        fillColor = "white",
        strokeColor = "black",
        withDashedStroke = false
    }) {
        let element = new joint.shapes.standard.Rectangle();
        let attributes = this.getAttributes_(
            elementName,
            fillColor,
            strokeColor,
            withDashedStroke,
            textAnchor,
            refX
        );

        attributes.body.rx = 8;
        attributes.body.ry = 8;

        element.resize(width, height);
        element.attr(attributes);

        return element;
    }

    buildBasicOctogonal(elementName, {
        width = this.defaultWidth,
        height = this.defaultHeight,
        textAnchor = "middle",
        refX = "42.5%",
        fillColor = "white",
        strokeColor = "black",
        withDashedStroke = false
    }) {
        let element = new joint.shapes.standard.Polygon();
        let attributes = this.getAttributes_(
            elementName,
            fillColor,
            strokeColor,
            withDashedStroke,
            textAnchor,
            refX
        );

        attributes.body.refPoints = '0,1 0.5,0 9.5,0 10,1 10,9 9.5,10 0.5,10 0,9';

        element.resize(width, height);
        element.attr(attributes);

        return element;
    }

    buildSmallCircle({fillColor = "black", strokeColor = "black"}) {
        let element = new joint.shapes.standard.Circle();
        let attributes = this.getAttributes_('', fillColor, strokeColor, false);

        element.resize(14, 14);
        element.attr(attributes);

        return element;
    }
}

module.exports = ShapeBuilder;
