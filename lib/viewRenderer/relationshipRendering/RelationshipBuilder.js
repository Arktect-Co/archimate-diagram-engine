...
class RelationshipBuilder {
  ...
  getRelationshipAttributes_(type, relationshipModelId, relationshipViewId, isBidirectional) {
    switch (type) {
      case 'association':
  ...
  buildRelationship(type, relationshipModelId, relationshipViewId, isBidirectional, bendpoints, sourceNode, targetNode, label) {
    ...
    link.attr(this.getRelationshipAttributes_(type, relationshipModelId, relationshipViewId, isBidirectional));
    ...
    if (bendpoints.length > 0) {
      ...
      for (let i = 0; i < len; i++) {
        let bendpoint = bendpoints[i];

        vertices.push(bendpoint);
      }
