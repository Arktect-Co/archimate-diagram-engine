...
import { archimateGlyphs } from '@lib/glyphGenerator/glyphsDescriptions/glyphs';
...
describe('glyphsDescriptions', () => {
  describe('ArchiMate <=3.1', () => {
    const glyphs = archimateGlyphs[ArchimateVersion.LessThanOrEqualV3_1];
  ...
  describe('Archimate 3.2', () => {
    const glyphs = archimateGlyphs[ArchimateVersion.V3_2];
    it('should return empty string if type does not contain glyph', () => {
      const glyph = getGlyph('unknow');
    ...
    it('should return a "applicationcomponent" glyph', () => {
      const type = NodeType.ApplicationComponent;
      const glyph = getGlyph(type, ArchimateVersion.V3_2);
